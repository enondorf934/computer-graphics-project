...
public final class UnitTree implements Drawable, Shape
{
  ...
  private void calculateTrunkVertices()
  {
    ...
    double center_x = w / 2;
    double trunk_width = TRUNK_WIDTH_RATIO * w;
    double trunk_height = TRUNK_HEIGHT_RATIO * h;

    // Calculate vertices to fill tree trunk
    trunkFillVertices.add(new Point2D.Double(center_x - (1.5 * trunk_width), y));
    trunkFillVertices.add(new Point2D.Double(center_x - (0.5 * trunk_width), y + trunk_width));
    trunkFillVertices.add(new Point2D.Double(center_x - (0.5 * trunk_width), y)); // remove for outline

    trunkFillVertices.add(new Point2D.Double(center_x - (0.5 * trunk_width), y + trunk_height));
    trunkFillVertices.add(new Point2D.Double(center_x + (0.5 * trunk_width), y + trunk_height));
